...
- Fixed redirect caching issue that required hard refresh after authentication
- Redirect now uses proper HTTP redirect via `template_redirect` hook instead of meta refresh

### Added

- Added no-cache headers to all redirect responses to prevent browser caching
...
- Regenerated autoloader with `--no-dev` flag to exclude test dependencies from production
...
- Added `--optimize-autoloader` flag to composer install in build process
